...
import { footballMatches } from '../dashboard/dashboardData'

function LiveFootballDetail() {
  const { id } = useParams()
  ...
  useEffect(() => {
    let mounted = true
    get('/api/external/football')
      .then((data) => {
        if (!mounted) return
        const matches = Array.isArray(data) ? data : []
        const idx = parseInt(id, 10)
        if (Number.isFinite(idx) && matches[idx]) setMatch(matches[idx])
        else if (Number.isFinite(idx) && footballMatches[idx]) setMatch(footballMatches[idx])
        else setMatch(null)
        setError(null)
      })
      .catch((err) => {
        ...
        if (!mounted) return
        const idx = parseInt(id, 10)
        if (Number.isFinite(idx) && footballMatches[idx]) setMatch(footballMatches[idx])
        else setMatch(null)
        setError(null)
      })
      ...
      <header className="live-topbar glass">
        <div className="live-topbar-inner">
          <div className="live-brand-copy">
            <span className="material-symbols-outlined live-brand-icon">sports_soccer</span>
            <div>
              <h1>{match.title}</h1>
                ...
                <div className="market-group" key={m.title + idx}>
                  ...
                  <div className="market-grid">
                    ...
                    <div className="market-col market-mid">
                      <div className="market-col-title">Value</div>
...
export default LiveFootballDetail
