...
function LiveCricketPanel() {
  return (
    <>
      {cricketMatches.map((match, matchIndex) => {
        const teams = match.title.split(' vs ')
        return (
          <article className="reference-match-card glass" id={matchIndex === 0 ? 'live-cricket' : undefined} key={match.title}>
            ...
            <div className="reference-match-score"><div><span className="team-badge">{teams[0].slice(0, 3).toUpperCase()}</span><small>{teams[0]}</small></div><strong>{match.score.split(' · ')[0]}</strong><div><span className="team-badge">{teams[1].slice(0, 3).toUpperCase()}</span><small>{teams[1]}</small></div></div>
